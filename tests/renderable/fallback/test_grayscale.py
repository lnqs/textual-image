...
def test_prepare() -> None:
    ...
    assert cast(PILImage.Image, renderable._prepared_image).size == (10, 11)
...
def test_calculate_image_size_for_render_size() -> None:
    renderable = Image(TEST_IMAGE)
    assert renderable._calculate_image_size_for_render_size(SIZE_TEN, TERM_SIZE) == (10, 11)
...
def test_get_pixel_failure() -> None:
    renderable = Image(TEST_IMAGE)
    assert renderable._get_pixel(0, 0) == " "
