"""Provides a Textual `Widget` to render images via the iTerm2 Inline Images Protocol in the terminal."""
...
class Image(BaseImage, Renderable=_NoopRenderable):
    """Textual `Widget` to render images via the iTerm2 Inline Images Protocol in the terminal."""
...
class _ImageITerm2Impl(Widget, can_focus=False, inherit_css=False):
    ...
    It is done in this child widget to simplify the process -- this class assumes it never has to render any borders or
    spacings, but only the parent will if required by the user.
    We assume `self.region == self.content_region` in this class, which lets us use the `crop` parameter
    in `render_lines()` directly on our image data without having to deal with gutters, as well as moving
    the cursor after rendering the iTerm2 data to an easily determinable position.
    """
