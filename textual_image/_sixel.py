...
def image_to_sixels(
    ...
    if _HAS_NUMPY:
        data, color_registers = _compact_palette_np(image, raw_data)
        chunks = _iter_bands_np(data, image.width, image.height, color_registers)
    else:
        data, color_registers = _compact_palette(image, raw_data)
        chunks = _iter_bands(data, image.width, image.height, color_registers)

    header = _make_header(image.width, image.height)
...
def _compact_palette(image: PILImage.Image, data: bytes) -> tuple[bytes, tuple[bytes, ...]]:
    ...
    remap, registers = _build_palette_map(palette, dict(Counter(data)))
...
def _make_header(width: int, height: int) -> bytes:
    ...
    return b'\x1bP0;0;0q"1;1;' + f"{width};{height}".encode("ascii")


@dataclass(slots=True)
class _ColorTracker:
    """Return ``#N;2;R;G;B`` on first use, ``#N`` on reuse, empty if already active."""
...
def _pack_band(
    ...
    for row in range(band_h):
        ...
        active.update(data[row_start : row_start + width])
    ...
    for row in range(band_h):
        ...
        for x, color in enumerate(row_bytes):
    ...
    return {c: (span_start[c], span_end[c]) for c in active}
...
def _iter_bands(
    ...
    for band_y in range(0, height, _BAND_HEIGHT):
        ...
        spans = _pack_band(data, band_y, band_h, width, band_data)
        bands.append(_emit_band(spans, band_data, tracker, band_h))
...
if _HAS_NUMPY:
    ...
    def _compact_palette_np(
        ...
        counts_arr = np.bincount(arr, minlength=MAX_COLORS)
    ...
    def _iter_bands_np(
        ...
        for band_idx in range(n_bands):
            ...
            for row in range(band_h):
                np.add.at(bitmask, (band[row], cols), _NP_BIT_WEIGHTS[row])

            # Identify active colors and compute horizontal spans
            active_colors = np.unique(band[:band_h])
    ...
    def _compact_palette_np(
        ...
        return _compact_palette(image, data)
    ...
    def _iter_bands_np(
        ...
        return _iter_bands(data, width, height, color_registers)
